...
//----------------------------------------------------------------------------------------------------------------------
// JoltPhysDriverBuilder
//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format JoltPhysDriverBuilder::getMaterialMemoryRequirements(uint32_t numMaterials)
{
  NMP::Memory::Format requirements(sizeof(MR::PhysicsMaterialDriverDataJoltPhys) * numMaterials, NMP_NATURAL_TYPE_ALIGNMENT);
  return requirements;
}

//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format JoltPhysDriverBuilder::getShapeMemoryRequirements(
  ME::PhysicsShapeExport::ShapeType NMP_UNUSED(type),
  uint32_t numShapes)
{
  NMP::Memory::Format requirements(sizeof(MR::PhysicsShapeDriverDataJoltPhys) * numShapes, NMP_NATURAL_TYPE_ALIGNMENT);
  return requirements;
}

//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format JoltPhysDriverBuilder::getPartMemoryRequirements(const ME::PhysicsPartExport* NMP_UNUSED(part))
{
  NMP::Memory::Format requirements(sizeof(MR::PhysicsActorDriverDataJoltPhys), NMP_NATURAL_TYPE_ALIGNMENT);
  requirements += NMP::Memory::Format(sizeof(MR::PhysicsBodyDriverDataJoltPhys), NMP_NATURAL_TYPE_ALIGNMENT);
  return requirements;
}

//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format JoltPhysDriverBuilder::getJointMemoryRequirements(const ME::PhysicsJointExport* NMP_UNUSED(joint))
{
  NMP::Memory::Format requirements(sizeof(MR::PhysicsJointDriverDataJoltPhys), NMP_NATURAL_TYPE_ALIGNMENT);
  return requirements;
}

//----------------------------------------------------------------------------------------------------------------------
void JoltPhysDriverBuilder::initMaterialDriverData(
  const ME::PhysicsShapeExport*       physicsMaterialExport,
  MR::PhysicsRigDef::Part::Material&  material,
  NMP::Memory::Resource&              resource)
{
  NMP::Memory::Format format(sizeof(MR::PhysicsMaterialDriverDataJoltPhys), NMP_NATURAL_TYPE_ALIGNMENT);
  MR::PhysicsMaterialDriverDataJoltPhys* driverData = (MR::PhysicsMaterialDriverDataJoltPhys*)resource.alignAndIncrement(format);
...
//----------------------------------------------------------------------------------------------------------------------
void JoltPhysDriverBuilder::initShapeDriverData(
  const ME::PhysicsShapeExport*           physicsShapeExport,
  MR::PhysicsRigDef::Part::Volume::Shape& shape,
  NMP::Memory::Resource&                  resource)
{
  NMP::Memory::Format format(sizeof(MR::PhysicsShapeDriverDataJoltPhys), NMP_NATURAL_TYPE_ALIGNMENT);
  MR::PhysicsShapeDriverDataJoltPhys* driverData = (MR::PhysicsShapeDriverDataJoltPhys*)resource.alignAndIncrement(format);
...
//----------------------------------------------------------------------------------------------------------------------
void JoltPhysDriverBuilder::initPartDriverData(
  const ME::PhysicsPartExport*  physicsPartExport,
  MR::PhysicsRigDef::Part&      part,
  NMP::Memory::Resource&        resource)
{
  NMP::Memory::Format actorFormat(sizeof(MR::PhysicsActorDriverDataJoltPhys), NMP_NATURAL_TYPE_ALIGNMENT);
  MR::PhysicsActorDriverDataJoltPhys* actorDriverData = (MR::PhysicsActorDriverDataJoltPhys*)resource.alignAndIncrement(actorFormat);
  ...
  NMP::Memory::Format bodyFormat(sizeof(MR::PhysicsBodyDriverDataJoltPhys), NMP_NATURAL_TYPE_ALIGNMENT);
  MR::PhysicsBodyDriverDataJoltPhys* bodyDriverData = (MR::PhysicsBodyDriverDataJoltPhys*)resource.alignAndIncrement(bodyFormat);
...
//----------------------------------------------------------------------------------------------------------------------
void JoltPhysDriverBuilder::initJointDriverData(
  const ME::PhysicsJointExport* physicsJointExport,
  MR::PhysicsJointDef*          jointDef,
  NMP::Memory::Resource&        resource)
{
  NMP::Memory::Format format(sizeof(MR::PhysicsJointDriverDataJoltPhys), NMP_NATURAL_TYPE_ALIGNMENT);
  MR::PhysicsJointDriverDataJoltPhys* driverData = (MR::PhysicsJointDriverDataJoltPhys*)resource.alignAndIncrement(format);
...
//----------------------------------------------------------------------------------------------------------------------
void JoltPhysDriverBuilder::dislocateDriverData(MR::PhysicsRigDef* physicsRigDef)
{
  MR::dislocateDriverDataJoltPhys(physicsRigDef);
}

JoltPhysDriverBuilder* g_driverBuilder = NULL;

#ifndef NMP_MEMORY_LOGGING
//----------------------------------------------------------------------------------------------------------------------
void loadPlugin(MR::Manager* manager, const NMP::Memory::Config* c)
{
  NMP_VERIFY_MSG(!g_driverBuilder, "Jolt Physics asset compiler plugin has already been initialised.");

  manager->registerAsset(MR::Manager::kAsset_PhysicsRigDef, MR::locatePhysicsRigDefJoltPhys);

  loadPluginPhysics(manager, c);

  g_driverBuilder = new JoltPhysDriverBuilder();
