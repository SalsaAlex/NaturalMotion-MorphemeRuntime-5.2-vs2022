...
//----------------------------------------------------------------------------------------------------------------------
  ...
  #pragma once
#endif
#ifndef MR_PHYSICS_SCENE_KARMA_H
#define MR_PHYSICS_SCENE_KARMA_H
//----------------------------------------------------------------------------------------------------------------------
...
#include "physics/Karma/mrKarma.h"
//----------------------------------------------------------------------------------------------------------------------

namespace MR
{

/// user data attached to Karma bodies etc must be of this type - see the mrPhysicsRigKarma.h for the declaration.
struct PhysicsRigKarmaBodyData;

class CharacterControllerInterface;
struct PhysicsSerialisationBuffer;
class PhysicsRigKarma;

//----------------------------------------------------------------------------------------------------------------------
/// \class PhysicsSceneKarma
///
/// Interface used to access the physics scene by morpheme runtime.
//----------------------------------------------------------------------------------------------------------------------
class PhysicsSceneKarma : public PhysicsScene
{
public:
  //----------------------------------------------------------------------
  // Functions that the application should call at the appropriate times
  //----------------------------------------------------------------------
  PhysicsSceneKarma(physx::PxScene* physXScene = 0);

  virtual ~PhysicsSceneKarma();
