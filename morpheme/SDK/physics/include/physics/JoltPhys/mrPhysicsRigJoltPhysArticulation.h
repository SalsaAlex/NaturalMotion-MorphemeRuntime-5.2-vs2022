...
//----------------------------------------------------------------------------------------------------------------------
  ...
  #pragma once
#endif
#ifndef MR_PHYSICS_RIG_JOLTPHYSARTICULATION_H
#define MR_PHYSICS_RIG_JOLTPHYSARTICULATION_H
...
typedef void (*ArticulationExplosionHandler)(
  class PhysicsRigJoltPhysArticulation* articulation,
  NMP::Matrix34& initTM);

//----------------------------------------------------------------------------------------------------------------------
/// \class MR::PhysicsRigJoltPhysArticulation
/// \brief Container for the Jolt Physics implementation of a physics rig def.
//----------------------------------------------------------------------------------------------------------------------
class PhysicsRigJoltPhysArticulation : public PhysicsRigJoltPhys
{
  enum { MAX_NAME_SIZE = 64 };
public:
  //--------------------------------------------------------------------------------------------------------------------
  class PartJoltPhysArticulation : public PartJoltPhys
  {
    friend class PhysicsRigJoltPhysArticulation;
  ...
  public:
    ...
    PartJoltPhysArticulation(int32_t defaultCollisionMask, int32_t allowedCollisionMask);
    PartJoltPhysArticulation(const PartJoltPhysArticulation& other);
    virtual ~PartJoltPhysArticulation();
    PartJoltPhysArticulation& operator=(const PartJoltPhysArticulation& other);
  ...
  //--------------------------------------------------------------------------------------------------------------------
  class JointJoltPhysArticulation : public JointJoltPhys
  {
    friend class PhysicsRigJoltPhysArticulation;

  public:

    JointJoltPhysArticulation(const PhysicsSixDOFJointDef* const def);
  ...
  PhysicsRigJoltPhysArticulation(PhysicsSceneJoltPhys* scene);
  ...
  /// Create an instance of a PhysicsRigv.
  ///
  ...
  PartJoltPhysArticulation          *getPartJoltPhysArticulation(uint32_t index)        {return (PartJoltPhysArticulation*)getPart(index);};
  const PartJoltPhysArticulation    *getPartJoltPhysArticulation(uint32_t index) const  {return (PartJoltPhysArticulation*)getPart(index);};
  JointJoltPhysArticulation         *getJointJoltPhysArticulation(uint32_t index)       {return (JointJoltPhysArticulation*)getJoint(index);};
  const JointJoltPhysArticulation   *getJointJoltPhysArticulation(uint32_t index) const {return (JointJoltPhysArticulation*)getJoint(index);};
...
private:
  ...
  static void createJoints(PhysicsSceneJoltPhys*physicsScene, PhysicsRigDef *physicsRigsDef,
    PhysicsRigJoltPhysArticulation *physXBody, PxArticulationLinkDesc **links);
